...
def process_frame_segmentation(frame, model):
    ...
    if not car_detections:
        ...
        # --- ALTERAÇÃO: Retorna o status "NONECAR" ---
        return display_frame, detected_class_names, "NONECAR"
...
def main():
    ...
    for filename in os.listdir(SOURCE_FOLDER):
        ...
        if file_ext not in allowed_extensions:
            print(f"\nIgnorando arquivo não-imagem: {filename}")
            continue
        ...
        try:
            ...
            if frame is None:
                raise FileNotFoundError(f"Não foi possível ler a imagem: {source_image_path}")
